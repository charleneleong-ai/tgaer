...
**Validation correction:** the replay **cannot** validate a fix. It feeds the *captured* frames, so the avatar positions are fixed at whatever the pre-fix agent produced (the design's stated "can't react" limit), and `replay_traces` asserts each replayed action equals the captured one — so a behaviour-changing fix makes it *diverge by design*. Fix validation needs a **closed-loop sim**, exactly how Phases 3b/4 were validated. Phase 6 ([`2026-06-26-explorer-phase6-affordance-cycle-fix.md`](2026-06-26-explorer-phase6-affordance-cycle-fix.md)) does this and lands the fix; the live re-sweep remains the real-world confirmation.
